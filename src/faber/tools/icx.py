...
def validate(cls, command, version, features):
    ...
    v = subprocess.check_output([command, '-V'], stderr=subprocess.STDOUT).decode()
...
class compile(action):
    ...
    command = 'icx $(cppflags) $(cflags) -c -o $(<) $(>)'
    cppflags = map(compiler.cppflags)
    cppflags += map(compiler.define, translate, prefix='-D')
    cppflags += map(compiler.include, translate, prefix='-I')
...
class link(action):
    ...
    def submit(self, targets, sources):
        ...
        src, linkpath, libs = icx.split_libs(sources)
...
class icx(cc):
    ...
    def __init__(self, name='icx', command=None, version='', features=()):

        command, version, features = validate(self.__class__, command or 'icx',
                                              version, features)
        cc.__init__(self, name=name, version=version)
        self.features |= features
        if command:
            self.makedep.subst('icx', command)
            self.compile.subst('icx', command)
            self.link.subst('icx', command)
