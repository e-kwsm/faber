...
from .cxx import *
from .icx import validate, makedep_wrapper


class makedep(action):

    command = 'icpx $(cppflags) -MM -o $(<) $(>)'
...
class compile(action):
    ...
    command = 'icpx $(cppflags) $(cxxflags) -c -o $(<) $(>)'
    cppflags = map(compiler.cppflags)
    cppflags += map(compiler.define, translate, prefix='-D')
    cppflags += map(compiler.include, translate, prefix='-I')
...
class link(action):
    ...
    def submit(self, targets, sources):
        ...
        src, linkpath, libs = icpx.split_libs(sources)
...
class icpx(cxx):
    ...
    def __init__(self, name='icpx', command=None, version='', features=()):

        command, version, features = validate(self.__class__, command or 'icpx',
                                              version, features)
        cxx.__init__(self, name=name, version=version)
        self.features |= features
        if command:
            self.makedep.subst('icpx', command)
            self.compile.subst('icpx', command)
            self.link.subst('icpx', command)
